...
Each Tracker block:
  ...
  2. Adds the Tracker's package root to sys.path.
...
# ── pyMultiTaskWS root (makes `multitrack` and `trackerWeb` importable) ───────
_pkg_root = str(Path(__file__).resolve().parent)
if _pkg_root not in sys.path:
    sys.path.insert(0, _pkg_root)

# ── TrackerWeb — platform smoke-test Tracker ──────────────────────────────────
# No credentials needed here; wsgi.py reads MULTITRACK_GPG_PASSPHRASE directly.
from trackerWeb.wsgi import application as web_app                      # noqa: E402
